...
Сервис ищется локально: сначала в реестре `services[module].services[name]`, затем абсолютным `require` из каталога `ext_modules` (и соседних модулей), а не относительным путём `../..` от бандла.
